...
from .classify import CHECKS, FINISH, FLOW_STEPS, RISKY, focus, gerund, label, meaningful_segments, noun, split_commands, strip_heredocs
...
def definition_of_done(groups, max_examples=3):
    ...
    stats = {}
    for f in FINISH:
        stats[f] = {"n": 0, "present": Counter(), "passed": Counter(), "tested_final": 0, "occurrences": []}
    for key, acts in groups.items():
        ...
        for i, (a, st) in enumerate(ev):
            ...
            passed = {s for (b, s) in window if s in CHECKS and b.outcome == "ok"}
            last_edit = max((j for j, (b, s) in enumerate(window) if s == "edit"), default=None)
            last_pass = max((j for j, (b, s) in enumerate(window) if s == "test" and b.outcome == "ok"), default=None)
            tested_final = last_pass is not None and (last_edit is None or last_pass > last_edit)
            rec = stats[st]
            rec["n"] += 1
            for c in present:
                rec["present"][c] += 1
            for c in passed:
                rec["passed"][c] += 1
            rec["tested_final"] += 1 if tested_final else 0
            rec["occurrences"].append((a, present, tested_final))
    out = {}
...
def suggestions(done, after, risk, secret, fric, loops, auto):
        ...
        if d and d["implicit_done"] and d["exceptions"]:
            ...
            out.append({
                ...
                "rule": {"id": "%s-before-%s" % ("-".join(checks), step), "type": "require_before",
                         "says": "%s pass%s before %s." % (names[:1].upper() + names[1:], "" if checks == ["test"] else "es", gerund(step)),
                         "match": {"steps": [step]}, "requires": checks, "passed": True, "if_missing": "hold"}})
    ...
    if held:
        out.append({
            ...
            "evidence": "; ".join("%s: %d times" % (label(k), risk[k]["ran"]) for k in held) + ".",
    ...
    if "force_push" in risk:
        out.append({"title": "Never force-push to main",
                    "evidence": "%d force-pushes in %d sessions." % (risk["force_push"]["attempts"], risk["force_push"]["sessions"]),
    ...
    if secret["commands"]:
        out.append({"title": "Keep secrets out of commands",
                    "evidence": "%d commands carried a literal secret (%s)." % (secret["commands"], ", ".join(sorted(secret["kinds"]))),
...
def discover(actions, since=None, until=None, max_examples=3):
    ...
    done = definition_of_done(groups, max_examples)
